...
const orderRoomReducer = createSlice({
  ...
  reducers: {
    setArrOrderRoomsAction: (
      state: InitialState,
      action: PayloadAction<orderRoom[]>
    ) => {
...
export const { setArrOrderRoomsAction, setInforRoomAction, setStatusAction } =
...
//Put DatPhong
export const editOrderRoomByIdApi = (id: number, roomInfor: orderRoom) => {
  return async (dispatch: AppDispatch) => {
    try {
      const result = await http.put(`/dat-phong/${id}`, roomInfor);
      dispatch(setStatusAction(result.status));
    } catch (err) {
      console.log(err);
    }
